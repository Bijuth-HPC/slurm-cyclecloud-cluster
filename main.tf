...
resource "azurerm_virtual_machine_extension" "install_cyclecloud" {
  ...
  settings = <<SETTINGS
    {
         "commandToExecute": "echo \"Launch Time: \" > /tmp/launch_time  && date >> /tmp/launch_time && curl -k -L -o /tmp/cyclecloud_install.py \"${local.cyclecloud_install_script_url}\" && python3 /tmp/cyclecloud_install.py --acceptTerms --useManagedIdentity --username=${var.cyclecloud_username} --password='${var.cyclecloud_password}' --publickey='${var.cyclecloud_user_publickey}' --storageAccount=${var.cyclecloud_storage_account} --webServerMaxHeapSize=4096M --webServerPort=80 --webServerSslPort=443 && sleep 1m &&  cyclecloud initialize --batch --url=https://localhost --verify-ssl=false --username=${var.cyclecloud_username} --password=${var.cyclecloud_password} && curl -k -L -o /tmp/slurm.txt \"${local.slurm_url}\" && cyclecloud import_cluster slurmprd01 -f /tmp/slurm.txt -c slurmprd01"
    }
SETTINGS
}
